...
class ImageInsertionForm(tk.Frame):
    ...
    def initialize(self):
        ...
        self.secret_image_dir = tk.StringVar()
        self.secret_image_dir.set('')
    ...
    def render_image_secret_frame(self):
        file_frame = hg.create_frame(self, self.SECRET_IMAGE_ROW + 1)

        hg.create_label(file_frame, 'Secret Image', 0, 0)
        hg.create_label(file_frame, self.secret_image_dir, 0, 1, fix_text=False)
        hg.create_button(file_frame, 'Choose',
                         lambda: self.load_image_secret_file(), 1, 0)

        hg.create_button(file_frame, 'Preview Image',
                         lambda: hg.show_image_secret_preview(self.secret_image_dir.get()), 1, 1)                         
    ...
    def load_image_file(self):
        dialog = fd.askopenfilename(
            filetypes=(("Image File", ('.bmp', '.png', '.jpeg', '.jpg')),)
        )
        self.image_dir.set(dialog)

    def load_image_secret_file(self):
        dialog = fd.askopenfilename(
            filetypes=(("Image File", ('.bmp', '.png', '.jpeg', '.jpg')),)
        )
        self.secret_image_dir.set(dialog)
